...
function getInterpretations(dayElement, dominant, counts) {
    ...
    if (dayElement === dominant) {
        summary += `특히 본인의 기운이 매우 강한 '신강(身强)' 사주로 판단됩니다. 이는 주관이 매우 뚜렷하고, 어떠한 난관이 닥쳐도 본인의 의지로 돌파해 나가는 추진력이 탁월함을 의미합니다.`;
    } else {
        summary += `주변 환경의 기운(${domName})이 본인을 이끌어주거나, 때로는 본인의 힘을 설기(기운을 뺌)시키는 구조를 가지고 있습니다. 이는 사회적인 적응력이 뛰어나고 주변 상황에 맞춰 유연하게 대처하는 능력을 의미합니다.`;
    }

    const personalityText = {
        wood: "성장과 의욕이 넘치며, 늘 새로운 것을 추구하는 창조적인 에너지가 가득합니다. 인자하고 부드러운 성품 속에 강한 자존심을 감추고 있어, 겉으로는 유해 보여도 속은 단단한 외유내강형입니다.",
        fire: "활활 타오르는 불처럼 열정이 넘치고 매사 분명한 것을 좋아합니다. 예의와 격식을 중시하며, 자신을 드러내고 표현하는 데에 주저함이 없습니다.",
        earth: "넓은 대지처럼 만물을 포용하는 넉넉한 마음씨와 믿음직스러운 신용을 지녔습니다. 쉽게 흔들리지 않는 뚝심이 있으며, 한 번 맺은 인연은 끝까지 지켜내는 의리가 있습니다.",
        metal: "단단한 바위나 날카로운 칼처럼 결단력이 있고 맺고 끊음이 확실합니다. 의(義)를 목숨보다 소중히 여기며, 옳지 않은 일에는 절대 타협하지 않는 강직함이 있습니다.",
        water: "흐르는 물처럼 어디에나 스며드는 유연함과 깊은 지혜를 지녔습니다. 총명하고 임기응변에 능하여 어떠한 환경에서도 살아남는 생존력이 탁월합니다."
    };
    ...
    ${counts[getWealthElementKey(dayElement)] > 0 
        ? "귀하의 사주 원국에는 재물의 기운이 뚜렷하게 자리 잡고 있습니다. 이는 타고난 재복(財福)이 있음을 의미하며, 경제적인 감각이 남들보다 뛰어납니다." 
        : "사주 원국에 재물의 기운이 약하게 드러나 있습니다. 이는 일확천금을 노리기보다는 본인의 전문 기술이나 재능(식상)을 발휘하여 차곡차곡 재산을 모아야 함을 암시합니다."}`;

    return {
        summary: summary,
        personality: `귀하의 사주에서 가장 강력한 기운은 <strong>${domName}</strong>입니다.<br><br>${personalityText[dominant]}<br><br>이러한 기운이 본원인 <strong>${selfName}</strong>와 결합하여, 귀하는 남들과 다른 독창적인 시각과 문제 해결 능력을 갖추게 되었습니다.`,
        ...
        ${Math.abs(counts.wood - counts.metal) < 2 
            ? "이는 감정의 기복이 적고 안정적인 연애를 선호함을 의미합니다. 상대방을 배려하고 이해하는 폭이 넓어 다툼이 적고, 오랜 기간 신뢰를 바탕으로 한 만남을 지속할 수 있습니다." 
            : "이는 사랑에 있어 매우 열정적이고 드라마틱한 성향을 보임을 의미합니다. 한 번 불타오르면 물불 가리지 않고 헌신하지만, 식을 때도 차갑게 돌아설 수 있습니다."}`,
        wealth: wealthLogic,
        caution: `인생의 흐름에서 가장 주의해야 할 것은 <strong>${domName}</strong> 기운의 과잉입니다.<br><br>
        ${dominant === 'fire' ? '불의 기운이 지나치면 성급함으로 인해 공든 탑을 무너뜨릴 수 있습니다.' : 
          dominant === 'water' ? '물의 기운이 범람하면 깊은 우울감이나 잡생각에 빠져 현실 도피적인 성향이 나타날 수 있습니다.' : 
          dominant === 'wood' ? '나무의 기운이 빽빽하면 고집불통이 되어 주변 사람들과 고립될 수 있습니다.' : 
          dominant === 'metal' ? '금의 기운이 강하면 날카로운 언행으로 인덕(人德)을 잃을 수 있습니다.' : 
          '토의 기운이 태과하면 게으름이나 무사안일주의에 빠질 수 있습니다.'}`
    };
}
...
function getCompatibility(dayElem, dominant) {
    ...
    if (dayElem === dominant) {
        ...
        good = {
            ...
            reason: `귀하는 <strong>${ELEMENT_NAMES_KO[dayElem]}</strong> 기운이 강하기 때문에, 이를 적절히 제어하고 다듬어주는 <strong>${ELEMENT_NAMES_KO[controlElem]}</strong> 기운이 필요합니다.`
        };
        const genElem = generating[dayElem];
        bad = {
            name: ELEMENT_NAMES_KO[genElem],
            reason: `이미 본인의 기운이 강한데 <strong>${ELEMENT_NAMES_KO[genElem]}</strong> 기운이 더해지면 고집이 세지고 독단적으로 변할 수 있습니다.`
        };
    } else {
        const genElem = generating[dayElem];
        good = {
            name: ELEMENT_NAMES_KO[genElem],
            reason: `귀하는 <strong>${ELEMENT_NAMES_KO[dayElem]}</strong> 기운이 약한 편이므로, 어머니처럼 나를 생(生)해주고 도와주는 <strong>${ELEMENT_NAMES_KO[genElem]}</strong> 기운이 절실합니다.`
        };
        const controlElem = controlling[dayElem];
        bad = {
            name: ELEMENT_NAMES_KO[controlElem],
            reason: `본인의 기운이 약한 상태에서 <strong>${ELEMENT_NAMES_KO[controlElem]}</strong> 기운이 들어오면, 심리적으로 위축되거나 건강을 해칠 수 있습니다.`
        };
    }
...
function getYearlyLuck(element, year) {
    const lucks = [
        "올해는 귀하의 인생에서 새로운 챕터가 시작되는 '변곡점'과 같은 시기입니다. 그동안 막혀있던 운의 물꼬가 트이며, 생각지도 못했던 곳에서 귀인(貴人)이 나타나 도움을 줄 것입니다.",
        "노력한 만큼의 정직한 결실을 맺는 '수확'의 해입니다. 요행을 바라기보다는 땀 흘려 일군 성과가 빛을 발할 것입니다.",
        "잠시 숨을 고르며 내실을 다져야 하는 '인내'의 시기입니다. 무리하게 일을 확장하거나 투자를 감행하기보다는, 현재 가진 것을 지키고 갈고닦는 데 집중해야 합니다.",
        "문서운이 강하게 들어오는 해입니다. 부동산 계약, 매매, 혹은 학업과 관련된 성취가 따를 것입니다. 오랫동안 골머리를 앓던 법적인 문제나 서류 문제가 시원하게 해결될 조짐이 보입니다.",
        "재물운이 폭발적으로 상승하는 '황금기'입니다. 뜻밖의 횡재수가 있으며, 투자했던 곳에서 큰 이익을 거둘 수 있습니다."
    ];
...
function getDailyLuck(stemIndex) {
    ...
    const msgs = [
        ...
        "주변 사람들과의 화합이 중요한 날입니다. 독단적인 결정보다는 경청하는 자세가 행운을 불러옵니다.",
        "작은 실수가 큰 오해를 부를 수 있으니 언행을 각별히 조심해야 합니다. 돌다리도 두드려보고 건너는 신중함이 액운을 막아줍니다.",
        "재물운이 좋은 날입니다. 생각지도 못한 용돈이 생기거나, 잊고 있던 돈을 찾을 수 있습니다.",
        ...
        "이동수가 있는 날입니다. 여행이나 출장을 가게 될 수 있으며, 그곳에서 새로운 기회를 발견하게 됩니다."
    ];
